...
**Date Implemented:** 2026-02-23 (Phase 1)
...
**Current Behavior:**
Phase 1 is implemented: Theme protocol, ThemeTokens (16 color slots), DefaultTheme, WhimsicalTheme, Glass system integration, CardView theming, Settings picker. However, each theme currently uses a narrow tonal palette (Whimsical uses only parchment/leather tones). The `SurfaceFill.textured` case is stubbed but not implemented. No background image support. No user-defined theme import/export. No additional built-in themes beyond Default and Whimsical.
...
### Phase 1: Theme Infrastructure (IMPLEMENTED — 2026-02-23)
...
**Phase 1 Test Steps:**
1. Launch app — should look identical to current (DefaultTheme active)
2. Open Settings > Display > Theme > select "Whimsical"
3. Verify Glass surfaces adopt warm parchment fill (non-visionOS) or remain material (visionOS)
4. Verify CardView adopts serif fonts, warmer shadows, softer corner radii
5. Verify GlassFormSection headers use theme headline font
6. Switch back to Default — verify clean revert to system appearance
7. Quit and relaunch — verify theme choice persists
8. Test on macOS and iOS — verify platform-appropriate adaptations
...
1. **Expand ThemeColors token set:**
   - `accentTertiary: Color` — third accent for visual variety (badges, tags, secondary buttons)
   - `surfaceTertiary: SurfaceFill` — third surface tone (alternating list rows, sidebar sections, navigation backgrounds)
   - `tagBackground: Color` — background for Kind badges and tags
   - `tagText: Color` — text color for Kind badges and tags
   - `destructive: Color` — delete/warning actions
   - `success: Color` — confirmation/positive states
   - Update `DefaultTheme` with sensible system-matching defaults for all new tokens
   - Update `WhimsicalTheme` with a richer multi-color palette using all new tokens

2. **Background image support:**
   - Add `ThemeBackgroundImages` struct to the `Theme` protocol with optional named image assets:
     - `sidebarBackground: String?` — tiled/stretched texture for the sidebar
     - `contentBackground: String?` — tiled texture for the main card list area
     - `murderboardCanvas: String?` — tiled texture for the Murderboard workspace
     - `structureBoardCanvas: String?` — tiled texture for the Structure Board
     - `wizardHero: String?` — decorative image for the Map Wizard landing
     - `emptyState: String?` — illustration for empty content states
     - `detailPlaceholder: String?` — hero/watermark for the "no selection" state
   - Each value is an optional asset name (nil = no image, use solid/material fill only)
   - Implement `SurfaceFill.textured` case fully (currently stub that ignores the texture name)
   - Create `ThemeBackgroundView` modifier that overlays an optional tiled or stretched image behind content
   - Wire into the 7 target surfaces in the appropriate view files

3. **Surfaces NOT targeted for background images (by design):**
   - Drawing Canvas — already has user-selectable canvas backgrounds for drawing
   - Settings Panel — should remain clean and system-standard
   - Toolbar — too small; materials work well here

4. **Enrich existing WhimsicalTheme:**
   - Expand palette beyond parchment/leather to include: aged gold for tags, forest green for success states, burgundy for destructive actions, warm cream alternating tones for surfaces
   - Add background image assets: parchment texture for sidebar, aged paper texture for content area, cork board texture for Murderboard canvas

5. **New built-in theme: Purple**
   - Deep plum (`#2D1B4E`) for primary surfaces
   - Royal violet (`#6B3FA0`) for accents
   - Lavender (`#C8A8E8`) for secondary surfaces
   - Silver (`#D0D0D8`) for text
   - Orchid (`#BA55D3`) for tertiary accents
   - Amethyst (`#9966CC`) for tag backgrounds
   - Wisteria (`#8E6BB5`) for borders
   - System default fonts (no serif)
   - Optional: subtle damask or velvet texture background images

6. **New built-in theme: Halloween**
   - Midnight black (`#0A0A0A`) for primary surfaces
   - Bone white (`#F5F0E0`) for text and card backgrounds
   - Pumpkin orange (`#E86C00`) for primary accent
   - Candy corn yellow (`#FFD700`) for secondary accent
   - Blood red (`#8B0000`) for destructive/tertiary accent
   - Charcoal (`#2D2D2D`) for secondary surfaces
   - System rounded fonts for a playful feel
   - Optional: spiderweb or haunted texture background images

**Phase 2 Design Approach:**
- Background images stored in Assets.xcassets under a `ThemeAssets/` group
- Built-in themes reference assets by name; user themes bundle their own images
- `ThemeBackgroundView` uses `Image(themeBgName).resizable().opacity(0.15)` pattern for subtle overlay
- Image rendering mode: `.tile` for textures, `.stretch` for hero/watermark images
- visionOS: background images are ignored (materials take precedence via `platformResolved`)

**Phase 2 Components Affected:**
- Modified: `Theming/ThemeTokens.swift` — add new color tokens + `ThemeBackgroundImages` struct
- Modified: `Theming/Theme.swift` — add `backgroundImages: ThemeBackgroundImages` to protocol
- Modified: `Theming/DefaultTheme.swift` — defaults for new tokens, nil background images
- Modified: `Theming/WhimsicalTheme.swift` — enriched multi-color palette + background image names
- New: `Theming/PurpleTheme.swift` — Purple theme definition
- New: `Theming/HalloweenTheme.swift` — Halloween theme definition
- Modified: `Theming/ThemeManager.swift` — register new themes
- New: `Theming/ThemeBackgroundView.swift` — reusable background image modifier
- Modified: `MainAppView.swift` — wire background images into sidebar + content area + detail placeholder
- Modified: `Murderboard/MurderBoardView.swift` — wire background image into canvas
- Modified: `StructureBoardView.swift` — wire background image into board
- Modified: `MapWizardView.swift` — wire background image into wizard landing
- New: Asset catalog entries under `ThemeAssets/` for built-in theme images
- Modified: `SettingsView.swift` — theme picker shows background image preview swatch

**Phase 2 Test Steps:**
1. Select Whimsical theme — verify enriched palette uses multiple distinct colors for surfaces, accents, tags, and chrome
2. Verify background textures appear on sidebar, content area, and Murderboard canvas
3. Verify background images are subtle (low opacity) and don't compete with content
4. Select Purple theme — verify cohesive purple palette across all surfaces, accents, text
5. Select Halloween theme — verify black/white/orange palette with bold contrast
6. Select Default theme — verify no background images, identical to pre-theme appearance
7. On visionOS — verify background images are suppressed and materials are used instead
8. Resize windows — verify background images tile/scale appropriately
...
1. **Theme file format: `.cumberlandtheme` (JSON)**
   - Registered `UTType` for import/export/sharing
   - `Codable` `UserTheme` struct conforming to `Theme` protocol
   - JSON schema includes all semantic tokens (colors, fonts, shapes, shadows, spacing, background images)
   - Colors specified as hex strings with light/dark mode variants: `{"light": "#F5E6C8", "dark": "#1E1914"}`
   - Background images bundled as base64-encoded PNG data within the JSON, or as companion files in a `.cumberlandtheme` bundle (directory package)

2. **Example `.cumberlandtheme` JSON structure:**
   ```json
   {
     "formatVersion": 1,
     "id": "my-custom-theme",
     ...
     "colors": {
       "surfacePrimary": {"type": "solid", "light": "#F5F5F0", "dark": "#1A1A1A"},
       "surfaceSecondary": {"type": "solid", "light": "#E8E8E0", "dark": "#2D2D2D"},
       "surfaceTertiary": {"type": "solid", "light": "#DDDDD5", "dark": "#3A3A3A"},
       "surfaceGlass": {"type": "material", "value": "ultraThinMaterial"},
       "surfaceGlassProminent": {"type": "material", "value": "thinMaterial"},
       "accentPrimary": {"light": "#4A90D9", "dark": "#5BA0E9"},
       "accentSecondary": {"light": "#357ABD", "dark": "#4A8AD0"},
       "accentTertiary": {"light": "#2C6BAF", "dark": "#3D7BC0"},
       "textPrimary": {"light": "#1A1A1A", "dark": "#F0F0F0"},
       "textSecondary": {"light": "#666666", "dark": "#AAAAAA"},
       "textTertiary": {"light": "#999999", "dark": "#777777"},
       "border": {"light": "#CCCCCC", "dark": "#444444"},
       "shadow": {"light": "#000000", "dark": "#000000"},
       "divider": {"light": "#DDDDDD", "dark": "#333333"},
       "cardBackground": {"light": "#FFFFFF", "dark": "#242424"},
       "highlightHairline": {"light": "#FFFFFF", "dark": "#555555"},
       "tagBackground": {"light": "#E0E8F0", "dark": "#2A3545"},
       "tagText": {"light": "#2C5F8A", "dark": "#8AB4E0"},
       "destructive": {"light": "#D32F2F", "dark": "#EF5350"},
     ...
     "shadows": {
       "cardColor": {"light": "#000000", "dark": "#000000"},
     ...
     "backgroundImages": {
       ...
       "murderboardCanvas": null,
       "structureBoardCanvas": null,
       "wizardHero": null,
       "emptyState": null,
...
3. **Import/Export workflow:**
   - Settings > Display > Theme section gains "Import Theme..." and "Export Theme..." buttons
   - Import: file picker filtered to `.cumberlandtheme` UTType; validates JSON; adds to ThemeManager
   - Export: saves the selected theme (including built-in ones) as a `.cumberlandtheme` file for sharing
   - Imported themes stored in app's Application Support directory
   - Imported themes persist across app launches
   - Share sheet integration for sending `.cumberlandtheme` files to other users

4. **Validation and fallback:**
   - Missing fields fall back to DefaultTheme values (partial themes are valid)
   - Invalid hex colors fall back to DefaultTheme equivalents
   - Invalid JSON shows user-friendly error alert with the parsing failure location
   - `formatVersion` field enables future schema evolution without breaking old themes

5. **Theme management in Settings:**
   - Imported/user themes shown in theme picker alongside built-in themes
   - Swipe-to-delete on imported themes (built-in themes cannot be deleted)
   - "Duplicate" action on any theme to create an editable copy
   - Theme count shown in section footer

**Phase 3 Design Approach:**
- `UserTheme` struct with all properties optional, resolved against `DefaultTheme` at load time
- `ThemeFileManager` handles import, export, persistence in Application Support
- `UTType.cumberlandTheme` declared in Info.plist with `public.json` conformance
- Background images in user themes: base64-encoded PNG data decoded at import time, cached to disk
- Large background images validated (max 2MB per image, max 4096x4096 resolution)

**Phase 3 Components Affected:**
- New: `Theming/UserTheme.swift` — Codable theme from JSON with DefaultTheme fallback
- New: `Theming/ThemeFileManager.swift` — import/export/persistence for user themes
- New: `Theming/ThemeJSONSchema.swift` — validation logic and error reporting
- Modified: `Theming/ThemeManager.swift` — load user themes from disk, add/remove user themes
- Modified: `SettingsView.swift` — Import/Export buttons, delete gesture on user themes
- Modified: `Info.plist` — UTType declaration for `.cumberlandtheme`

**Phase 3 Test Steps:**
1. Export DefaultTheme as `.cumberlandtheme` file — verify valid JSON with all fields
2. Edit exported JSON: change colors, save
3. Import modified theme — verify it appears in theme picker
4. Select imported theme — verify custom colors apply throughout the app
5. Quit and relaunch — verify imported theme persists and is still selectable
6. Import a theme with missing fields — verify it loads with DefaultTheme fallbacks for missing values
7. Import invalid JSON — verify user-friendly error alert
8. Delete an imported theme — verify it's removed from picker, app falls back to Default if it was active
...
**Notes:**
- Phase 1: No bundled custom fonts — uses system serif (`.design(.serif)`) to avoid licensing issues
- Phase 2: Background images should be created at 2x resolution for Retina displays
- Phase 2: visionOS ignores background images (materials required for spatial depth perception)
- Phase 3: User theme JSON format is designed to be forward-compatible — unknown keys are ignored, missing keys use defaults
- Per-Kind accent colors (`Kinds.accentColor(for:)`) remain unchanged across all phases — these are content-semantic, not theme-semantic
- Remaining views (CardSheetHeaderView, StructureBoardView, MurderBoardView, SidebarPanel, MainAppView) will be fully themed during Phase 2 background image integration
- Consider a future "Theme Gallery" where users browse and download community-created themes (post-v1.0)
