...
**Rationale:**
Cumberland's current UI uses default SwiftUI styling. A theming system would allow the app to present a more distinctive, "whimsical" aesthetic befitting a worldbuilding tool — evoking the feel of a writer's enchanted notebook — while preserving the underlying architecture. A proper theming system also opens the door for future skins (dark parchment, sci-fi, minimalist, etc.) without structural changes.

**Current Behavior:**
Default SwiftUI styling throughout. Colors, fonts, button styles, and backgrounds all use system defaults. No centralized theme management exists.

**Desired Behavior:**
A `ThemeManager` (Observable) propagated via SwiftUI Environment provides semantic design tokens consumed by all views. Switching themes changes the entire app's look and feel at runtime. The first delivered skin ("Whimsical") gives Cumberland a warm, handcrafted aesthetic.

**Implementation Summary:**
- **Phase 1:** Theme infrastructure — `Theme` protocol, `ThemeTokens` (semantic design tokens: colors, fonts, shapes, shadows, spacing), `SurfaceFill` enum (`.material`, `.solid`, `.textured`), `ThemeManager` (@Observable with @AppStorage persistence), `ThemeEnvironment` (EnvironmentKey injection)
- **Phase 2:** Glass system integration — All 6 Glass files (`GlassKit.swift`, `GlassEffects.swift`, `GlassSurfaceStyle.swift`, `GlassToolbarStyle.swift`, `GlassDropZone.swift`, `Components/GlassCard.swift`) read theme tokens via `@Environment(\.theme)` instead of hardcoded materials/colors/shadows
- **Phase 3:** `WhimsicalTheme` definition — parchment (#F5E6C8), ink (#2C1810), leather (#8B4513), muted gold (#B8860B) palette; system serif (.serif design) fonts; softer corner radii (16pt); warm brown shadows
- **Phase 4:** Theme picker in Settings > Display section with color swatch previews
- **Phase 5:** CardView themed — fonts, colors, corner radii, shadows all read from theme tokens
- **visionOS safety:** `SurfaceFill.platformResolved` ensures non-material fills are promoted to materials on visionOS for spatial depth perception
- **Zero regression guarantee:** `DefaultTheme` returns exact values from pre-existing hardcoded code
...
**Notes:**
- No bundled custom fonts in v1 — uses system serif (`.design(.serif)`) to avoid licensing issues
- No texture overlay images in v1 — `SurfaceFill.textured` case exists for future use
- Per-Kind accent colors (`Kinds.accentColor(for:)`) are unchanged — these are content-semantic, not theme-semantic
- Future themes can be added by conforming to `Theme` protocol and registering in `ThemeManager.init()`
- Remaining views (CardSheetHeaderView, StructureBoardView, MurderBoardView, SidebarPanel, MainAppView) can be incrementally themed — the Glass system changes already cascade to all callsites using Glass modifiers
