...
protocol GestureTarget: AnyObject {
...
struct DropProposal {
    ...
    static let cancel = DropProposal(operation: .cancel)
    static let copy = DropProposal(operation: .copy)
    static let move = DropProposal(operation: .move)
    static let forbidden = DropProposal(operation: .forbidden)
}
...
enum GestureEvent: CustomStringConvertible {
...
struct CoordinateSpaceInfo {
    ...
    /// World ← View: w = (v - T) / S
    func toWorldSpace(_ point: CGPoint) -> CGPoint {
        CGPoint(
    ...
    /// View ← World: v = w*S + T
    func fromWorldSpace(_ point: CGPoint) -> CGPoint {
        CGPoint(
    ...
    func hitTest(point: CGPoint, worldRect: CGRect) -> Bool {
...
struct PopupMenuItem {
    ...
    init(title: String, systemImage: String? = nil, isDestructive: Bool = false, isDisabled: Bool = false, action: @escaping () -> Void) {
        self.title = title
...
struct PopupMenu {
    ...
    let coordinateSpace: CoordinateSpaceInfo
    
    init(items: [PopupMenuItem], position: CGPoint, coordinateSpace: CoordinateSpaceInfo) {
        self.items = items
...
@MainActor
class MultiGestureHandler: ObservableObject {
    ...
    var zoomCenterMode: ZoomCenterMode = .pointer
    
    // MARK: - Popup System
    
    /// Currently active popup menu
    @Published var activePopup: PopupMenu?
    ...
    @Published private(set) var isTwoFingerPanning = false
    ...
    var currentPointerLocation: CGPoint { lastPointerLocation }
    ...
    func registerTarget(_ target: GestureTarget) {
    ...
    func unregisterTarget(_ target: GestureTarget) {
        targets.removeAll { $0.target?.gestureID == target.gestureID }
    ...
    func findDropTarget(at location: CGPoint, coordinateInfo: CoordinateSpaceInfo, acceptingTypes: [UTType]) -> DropCapableTarget? {
        cleanupWeakReferences()
    ...
    func processDragEntered(at location: CGPoint, session: DropSession, coordinateInfo: CoordinateSpaceInfo) {
    ...
    func processDragUpdated(at location: CGPoint, session: DropSession, coordinateInfo: CoordinateSpaceInfo) -> DropProposal {
        updatePointerLocation(location)
        let typesFromSession = extractTypesFromSession(session)
    ...
    func processPerformDrop(at location: CGPoint, session: DropSession, coordinateInfo: CoordinateSpaceInfo) -> Bool {
        updatePointerLocation(location)
        defer {
    ...
    func showPopup(_ menu: PopupMenu) {
        activePopup = menu
    ...
    func computeZoomCenter(fallbackLocation: CGPoint, coordinateInfo: CoordinateSpaceInfo) -> CGPoint {
...
extension MultiGestureHandler {
    ...
    func processDragBegan(location: CGPoint, coordinateInfo: CoordinateSpaceInfo) {
        updatePointerLocation(location)
    ...
    func processDragChanged(location: CGPoint, coordinateInfo: CoordinateSpaceInfo) {
        updatePointerLocation(location)
    ...
    func processRightClick(location: CGPoint, coordinateInfo: CoordinateSpaceInfo) -> Bool {
        updatePointerLocation(location)
    ...
    func processPinchBegan(scale: CGFloat, location: CGPoint, coordinateInfo: CoordinateSpaceInfo) {
        isPinching = true
        updatePointerLocation(location)
    ...
    func processPinchChanged(scale: CGFloat, location: CGPoint, coordinateInfo: CoordinateSpaceInfo) {
        updatePointerLocation(location)
    ...
    func processPinchEnded(scale: CGFloat, location: CGPoint, coordinateInfo: CoordinateSpaceInfo) {
        updatePointerLocation(location)
        guard isPinching else {
    ...
    func processTwoFingerPanBegan(location: CGPoint, coordinateInfo: CoordinateSpaceInfo) {
    ...
    func processTwoFingerPanChanged(translation: CGSize, location: CGPoint, coordinateInfo: CoordinateSpaceInfo) {
        updatePointerLocation(location)
    ...
    func processTwoFingerPanEnded(translation: CGSize, velocity: CGSize, location: CGPoint, coordinateInfo: CoordinateSpaceInfo) {
        updatePointerLocation(location)
        guard isTwoFingerPanning else {
...
extension View {
    ...
    func multiGestureHandler(_ handler: MultiGestureHandler, coordinateInfo: CoordinateSpaceInfo) -> some View {
