...
struct StoryStructureDiagnosticsView: View {
    ...
    private func structureName(for element: StructureElement) -> String {
        ...
        return structureNameByID[element.storyStructure?.persistentModelID ?? .init()] ?? "—"
    }
...
#Preview("Story Structure Diagnostics – Sample") {
    let container = try! ModelContainer(for: [StoryStructure.self, StructureElement.self], configurations: ModelConfiguration(isStoredInMemoryOnly: true))
    let ctx = container.mainContext
    let s1 = StoryStructure(name: "Three-Act")
    let s2 = StoryStructure(name: "Hero’s Journey")
    let e1 = StructureElement(name: "Act I", orderIndex: 0); e1.storyStructure = s1
    let e2 = StructureElement(name: "Act II", orderIndex: 1); e2.storyStructure = s1
    let e3 = StructureElement(name: "Act III", orderIndex: 2); e3.storyStructure = s1
    let e4 = StructureElement(name: "Call to Adventure", orderIndex: 0); e4.storyStructure = s2
    let e5 = StructureElement(name: "Refusal of the Call", orderIndex: 1); e5.storyStructure = s2
    [s1, s2, e1, e2, e3, e4, e5].forEach { ctx.insert($0) }
    try? ctx.save()

    return StoryStructureDiagnosticsView()
