...
public struct SeededRandomGenerator {
    ...
    public mutating func nextDouble(in range: ClosedRange<CGFloat>) -> CGFloat {
...
public class BrushEngine {
    ...
    public static func createPKTool(from brush: MapBrush, color: Color, width: CGFloat? = nil) -> PKTool {
        ...
        #if canImport(UIKit)
        // DR-0001: Use a resolved color to prevent black color inversion in PencilKit on iOS.
        // PencilKit can invert black (#000000) in dark mode, so we resolve the color to
        // a near-black (e.g. #010101) when the color is pure black to avoid this.
        let uiColor = UIColor(brushColor)
        var red: CGFloat = 0, green: CGFloat = 0, blue: CGFloat = 0, alpha: CGFloat = 0
        uiColor.getRed(&red, green: &green, blue: &blue, alpha: &alpha)
        let isPureBlack = red == 0 && green == 0 && blue == 0
        let pkColor = isPureBlack
            ? UIColor(red: 1/255, green: 1/255, blue: 1/255, alpha: alpha)
            : uiColor
    ...
    public static func renderStroke(
        brush: MapBrush,
        points: [CGPoint],
        color: Color,
        width: CGFloat? = nil,
    ...
    public static func dottedStroke(
        points: [CGPoint],
        color: Color,
        width: CGFloat,
        context: CGContext
    ...
    public static func renderSolidStroke(
        points: [CGPoint],
        color: Color,
        width: CGFloat,
        brush: MapBrush,
    ...
    public static func renderVariableWidthStroke(
        points: [CGPoint],
        color: Color,
        width: CGFloat,
        brush: MapBrush,
    ...
    public static func renderDashedStroke(
        points: [CGPoint],
        color: Color,
        width: CGFloat,
        context: CGContext
    ...
    public static func renderDottedStroke(
        points: [CGPoint],
        color: Color,
        width: CGFloat,
        context: CGContext
    ...
    public static func renderStippledStroke(
        points: [CGPoint],
        color: Color,
        width: CGFloat,
        context: CGContext
    ...
    public static func renderStampStroke(
        points: [CGPoint],
        brush: MapBrush,
        color: Color,
        width: CGFloat,
    ...
    public static func renderHatchedStroke(
        points: [CGPoint],
        color: Color,
        width: CGFloat,
        context: CGContext
    ...
    public static func renderCrossHatchedStroke(
        points: [CGPoint],
        color: Color,
        width: CGFloat,
        context: CGContext
    ...
    public static func smoothPath(_ points: [CGPoint], amount: CGFloat) -> [CGPoint] {
    ...
    public static func snapToGrid(_ points: [CGPoint], gridSpacing: CGFloat) -> [CGPoint] {
        points.map { point in
            CGPoint(
    ...
    public static func renderAdvancedStroke(
        brush: MapBrush,
        points: [CGPoint],
        color: Color,
        width: CGFloat? = nil,
    ...
    public static func renderTerrainBrush(
        brush: MapBrush,
        points: [CGPoint],
        color: Color,
        width: CGFloat,
    ...
    public static func calculateScaleAwareBeachWidth(
        brushWidth: CGFloat,
    ...
    public static func renderWaterBrush(
        brush: MapBrush,
        points: [CGPoint],
        color: Color,
        width: CGFloat,
        ...
        if brush.name.lowercased().contains("coast") {
            ...
            // ER-0007 Phase 2: Elevation-based river rendering with valleys
            renderRiverBrush(brush: brush, points: points, color: color, width: width, context: context, terrainMetadata: terrainMetadata)
    ...
    /// ER-0007 Phase 1: Elevation-based lake rendering (matches base layer exactly)
    public static func renderLakeBrush(
        brush: MapBrush,
        points: [CGPoint],
        color: Color,
        width: CGFloat,
        ...
        // Step 1: Create smooth path from points (lake boundary)
        ...
        for i in 1..<points.count {
            ...
            if i < points.count - 1 {
                ...
                lakePath.addQuadCurve(to: midPoint, control: current)
            } else {
                lakePath.addLine(to: current)
            }
        }
        lakePath.closeSubpath()

        // Step 2: Calculate bounding box
        let boundingBox = lakePath.boundingBoxOfPath
        let mapWidth = Int(boundingBox.width)
        let mapHeight = Int(boundingBox.height)

        guard mapWidth > 0, mapHeight > 0 else { return }

        // Step 3: Get terrain parameters
        let seed = Int.random(in: 0...10000)
        let mapScale = terrainMetadata?.physicalSizeMiles ?? 10.0

        // Determine noise parameters based on map scale (same as base layer)
        let (noiseScale, octaves): (Double, Int)
        if mapScale >= 1000 {
            (noiseScale, octaves) = (2.0, 7)
        } else if mapScale >= 100 {
            let t = (mapScale - 100) / 900
            let scale = 1.0 + t
            let oct = 5 + Int(t * 2)
            (noiseScale, octaves) = (scale, oct)
        } else {
            let t = (mapScale - 1) / 99
            let scale = 0.3 + (t * 0.7)
            let oct = 3 + Int(t * 2)
        ...
        // Step 4: Generate elevation map for lake region
        // Use 60% water (lakes are mostly water with sandy beaches)
        ...
        // Step 5: Create noise generator for texture variation
        let textureNoise = NoiseGenerator(seed: seed + 1000)

        // Step 6: Set up clipping path
        // NOTE: When called via renderAndCacheStroke, points are already in local (0-based) coordinates
        context.saveGState()
        context.addPath(lakePath)
        context.clip()

        // Step 7: Render pixel-by-pixel directly to provided context
        for y in stride(from: 0, to: mapHeight, by: 2) {
            for x in stride(from: 0, to: mapWidth, by: 2) {
                let elevation = elevationMap.elevationInterpolated(
                    at: CGPoint(x: Double(x), y: Double(y))
                )

                let noise = textureNoise.noise2D(x: Double(x) / 100.0, y: Double(y) / 100.0)

                let pixelColor: Color
                if elevation < 0 {
                    let depth = -elevation
                    let depthBrightness = 0.7 - (depth * 0.4)
                    let noiseBrightness = 1.0 + (noise * 0.05)
                    let finalBrightness = max(0.2, min(0.8, depthBrightness * noiseBrightness))
                    pixelColor = Color(hue: 0.55, saturation: 0.70, brightness: finalBrightness)
                } else if elevation < 0.1 {
                    pixelColor = Color(red: 0.90, green: 0.75, blue: 0.50).adjustedBrightness(1.0 + noise * 0.05)
                } else {
                    let green = Color(red: 0.45, green: 0.70, blue: 0.30)
                    pixelColor = green.adjustedBrightness(1.15 - (elevation * 0.30) + (noise * 0.05))
                }

                #if canImport(UIKit)
                let cgColor = UIColor(pixelColor).cgColor
                #elseif canImport(AppKit)
                let cgColor = NSColor(pixelColor).cgColor
                #else
                let cgColor = CGColor(red: 0.5, green: 0.5, blue: 0.8, alpha: 1)
                #endif

                context.setFillColor(cgColor)
                context.fill(CGRect(x: boundingBox.origin.x + CGFloat(x), y: boundingBox.origin.y + CGFloat(y), width: 2, height: 2))
            }
        }

        // Step 8: Restore graphics state
        context.restoreGState()
    }

    /// ER-0007 Phase 2 REVISED: Elevation-based river rendering with valleys and caching
    /// This function now caches the rendered result for performance
    public static func renderRiverBrush(
        brush: MapBrush,
        points: [CGPoint],
        color: Color,
        width: CGFloat,
        ...
        // Step 2: Calculate river width (moderate scaling)
        let baseRiverWidth = width * (1.0 + log10(max(1.0, pathLength / 100.0)) * 0.3)

        // Step 3: Generate meandered path
        let seed = Int.random(in: 0...10000)
        let meanderIntensity: CGFloat = 1.5
        let meanderedPath = generateProceduralRiverPath(
            points: points,
            width: baseRiverWidth,
            meanderIntensity: meanderIntensity,
            seed: seed
        )

        // Step 4: Calculate valley width (RESTORED TO ORIGINAL WIDER LOOK)
        // User feedback: User LIKED the wider valley look
        let valleyMultiplier = 5.0 + (10.0 * min(1.0, Double(pathLength) / 2000.0))
        let valleyWidth = baseRiverWidth * CGFloat(valleyMultiplier)
        ...
        for point in meanderedPath {
            minX = min(minX, point.x - valleyWidth)
            minY = min(minY, point.y - valleyWidth)
            maxX = max(maxX, point.x + valleyWidth)
            maxY = max(maxY, point.y + valleyWidth)
        }

        let mapWidth = Int(maxX - minX)
        let mapHeight = Int(maxY - minY)

        guard mapWidth > 0, mapHeight > 0 else { return }

        // Step 6: Create noise generator
        let noise = NoiseGenerator(seed: seed + 2000)

        // Step 7: Render valley context (flood plain, beaches, grass) - NOT water
        // Water will be drawn as a guaranteed continuous stroke afterward
        for y in stride(from: 0, to: mapHeight, by: 2) {
            for x in stride(from: 0, to: mapWidth, by: 2) {
                let worldX = minX + CGFloat(x)
                let worldY = minY + CGFloat(y)
                let pixelPoint = CGPoint(x: worldX, y: worldY)

                // Find distance to nearest point on river SEGMENTS
                var minDist = CGFloat.infinity
                for i in 0..<(meanderedPath.count - 1) {
                    let segmentStart = meanderedPath[i]
                    let segmentEnd = meanderedPath[i + 1]
                ...
                // Skip if inside the river water area (will be drawn as continuous stroke)
                if minDist < baseRiverWidth {
                    continue  // Inside river water - skip
                }

                // Normalize distance for valley
                let normalizedDist = min(1.0, minDist / valleyWidth)

                if normalizedDist >= 1.0 {
                    continue  // Outside valley
                }

                // Calculate elevation for valley context
                let distanceElevation = (normalizedDist * 0.6) - 0.3

                let noiseValue = noise.fractalNoise2D(
                    x: Double(worldX) / 50.0,
                    y: Double(worldY) / 50.0,
                    octaves: 4,
                    persistence: 0.5
                )

                let elevation = distanceElevation * 0.7 + noiseValue * 0.3

                // Determine color (ONLY flood plain, beaches, grass - NO water)
                let pixelColor: Color
                if elevation < 0.05 {
                    // Flood plain / wet basin
                    let wetness = max(0, min(1.0, (0.05 - elevation) / 0.15))
                    pixelColor = Color(red: 0.75 - (wetness * 0.10), green: 0.70 - (wetness * 0.05), blue: 0.55 - (wetness * 0.05))
                } else if elevation < 0.2 {
                    // Sandy banks / dry basin
                    pixelColor = Color(red: 0.90, green: 0.75, blue: 0.50).adjustedBrightness(1.0 + noiseValue * 0.05)
                } else {
                    // Valley walls / grassland
                    let green = Color(red: 0.45, green: 0.70, blue: 0.30)
                    pixelColor = green.adjustedBrightness(1.15 - ((elevation - 0.2) * 0.4) + (noiseValue * 0.05))
                }
        ...
        // Step 8: Draw guaranteed continuous water stroke
        // Create a path from the meandered points
        let riverPath = CGMutablePath()
        if let firstPoint = meanderedPath.first {
            riverPath.move(to: firstPoint)
            for i in 1..<meanderedPath.count {
                riverPath.addLine(to: meanderedPath[i])
            }
        }

        // Set water color (deep river blue)
        let waterColor = Color(hue: 0.55, saturation: 0.70, brightness: 0.50)
        #if canImport(UIKit)
        ...
        // Draw the continuous water stroke
        context.setStrokeColor(waterCGColor)
        context.setLineWidth(baseRiverWidth * 2.0)  // Full river width
        context.setLineCap(.round)
        context.setLineJoin(.round)
    ...
    public static func distanceToLineSegment(point: CGPoint, lineStart: CGPoint, lineEnd: CGPoint) -> CGFloat {
    ...
    public static func renderMarshBrush(
        brush: MapBrush,
        points: [CGPoint],
        color: Color,
        width: CGFloat,
    ...
    public static func renderVegetationBrush(
        brush: MapBrush,
        points: [CGPoint],
        color: Color,
        width: CGFloat,
    ...
    public static func renderRoadBrush(
        brush: MapBrush,
        points: [CGPoint],
        color: Color,
        width: CGFloat,
    ...
    public static func applyGridSnapping(_ points: [CGPoint], gridSize: CGFloat) -> [CGPoint] {
    ...
    public static func renderStructureBrush(
        brush: MapBrush,
        points: [CGPoint],
        color: Color,
        width: CGFloat,
    ...
    public static func renderPatternBasedBrush(
        brush: MapBrush,
        points: [CGPoint],
        color: Color,
        width: CGFloat,
    ...
    public static func renderSmartWall(
        points: [CGPoint],
        width: CGFloat,
        context: CGContext,
        brushName: String
    ...
    public static func renderStraightWall(
        points: [CGPoint],
        width: CGFloat,
        context: CGContext
    ) {
    ...
    public static func renderThinWall(
        points: [CGPoint],
        width: CGFloat,
        context: CGContext
    ) {
    ...
    public static func renderThickWall(
        points: [CGPoint],
        width: CGFloat,
        context: CGContext
    ) {
    ...
    public static func renderStandardWallSegment(
        from start: CGPoint,
        to end: CGPoint,
        width: CGFloat,
        context: CGContext
    ...
    public static func renderJaggedWall(
        points: [CGPoint],
        width: CGFloat,
        context: CGContext
    ) {
    ...
    public static func renderStampPattern(
        points: [CGPoint],
        width: CGFloat,
        spacing: CGFloat,
        context: CGContext,
    ...
    public static func renderStampShape(
        at position: CGPoint,
        size: CGFloat,
        angle: CGFloat,
        context: CGContext,
    ...
    public static func renderHatchedPattern(
        points: [CGPoint],
        width: CGFloat,
        context: CGContext
    ) {
    ...
    public static func renderStippledPattern(
        points: [CGPoint],
        width: CGFloat,
        context: CGContext
    ) {
    ...
    public static func renderSolidAreaFill(
        points: [CGPoint],
        width: CGFloat,
        context: CGContext
    ) {
    ...
    public static func renderTexturedPattern(
        points: [CGPoint],
        width: CGFloat,
        context: CGContext
    ) {
    ...
    public static func recommendedRenderingMethod(for brush: MapBrush) -> BrushRenderingMethod {
    ...
    // MARK: - Stroke Caching (ER-0007 Performance Fix)

    /// Render an advanced stroke and return cached image data for performance
    /// Returns (imageData, origin) tuple for storing in DrawingStroke
    public static func renderAndCacheStroke(
        brush: MapBrush,
        points: [CGPoint],
        color: Color,
        width: CGFloat,
        terrainMetadata: TerrainMapMetadata? = nil
    ) -> (data: Data, origin: CGPoint)? {
        ...
        for point in points {
            minX = min(minX, point.x - width * 5)  // Extra margin for valley
            minY = min(minY, point.y - width * 5)
            maxX = max(maxX, point.x + width * 5)
            maxY = max(maxY, point.y + width * 5)
        }

        let boundingWidth = Int(ceil(maxX - minX))
        let boundingHeight = Int(ceil(maxY - minY))
        let origin = CGPoint(x: minX, y: minY)

        guard boundingWidth > 0, boundingHeight > 0 else { return nil }
        ...
        guard let context = CGContext(
            ...
            bitsPerComponent: 8,
            bytesPerRow: boundingWidth * 4,
            space: colorSpace,
            bitmapInfo: bitmapInfo
        ) else { return nil }

        // Translate points to local coordinate system
        let localPoints = points.map { CGPoint(x: $0.x - minX, y: $0.y - minY) }

        // Render the stroke
        renderAdvancedStroke(
            brush: brush,
            points: localPoints,
            color: color,
            width: width,
    ...
    /// Draw a cached stroke image
    public static func drawCachedStroke(imageData: Data, origin: CGPoint, context: CGContext) {
        #if os(macOS)
        ...
        context.draw(cgImage, in: CGRect(x: origin.x, y: origin.y, width: CGFloat(width), height: CGFloat(height)))
    }
}

// MARK: - Brush Rendering Method

public enum BrushRenderingMethod {
...
extension Color {
    /// ER-0007: Adjust color brightness while preserving hue and saturation
    /// - Parameter brightness: Brightness multiplier (0.0 = black, 1.0 = original, > 1.0 = brighter)
    /// - Returns: Adjusted color
    public func adjustedBrightness(_ brightness: Double) -> Color {
        #if canImport(UIKit)
...
// clamped(to:) extension on CGFloat is defined in TerrainPattern.swift
