...
public struct TerrainPattern: ProceduralPattern {
    public let metadata: TerrainMapMetadata
    public let dominantFillType: BaseLayerFillType

    public init(metadata: TerrainMapMetadata, dominantFillType: BaseLayerFillType) {
        self.metadata = metadata
        self.dominantFillType = dominantFillType
    }

    public func draw(in context: CGContext, rect: CGRect, seed: Int, baseColor: Color, mapScale: Double?) {
    ...
    public func calculateWorldElevationScale(physicalSizeMiles: Double) -> Double {
    ...
    public func calculateNoiseParameters(physicalSizeMiles: Double) -> (Double, Int) {
        if physicalSizeMiles >= 1000 {
    ...
    public func meanColorFor(terrainType: BaseLayerFillType) -> Color {
        switch terrainType {
        case .sandy:
    ...
    public func colorForWater(depth: Double, noise: Double) -> Color {
    ...
    public func colorForTerrainComposition(terrainType: BaseLayerFillType, elevation: Double, noise: Double) -> Color {
    ...
    public func compositionWaterType(elevation: Double, noise: Double) -> Color {
        if elevation < 0.1 {
            // Sandy beach
    ...
    public func compositionLandType(elevation: Double, noise: Double) -> Color {
    ...
    public func compositionSandyType(elevation: Double, noise: Double) -> Color {
        if elevation < 0.7 {
    ...
    public func compositionForestedType(elevation: Double, noise: Double) -> Color {
        // ER-0007: Standardized threshold to 0.1 for consistency with all terrain types
        if elevation < 0.1 {
            // Sandy shores
            return Color(red: 0.90, green: 0.75, blue: 0.50).adjusted(brightness: 1.0 + noise * 0.05)
    ...
    public func compositionMountainType(elevation: Double, noise: Double) -> Color {
        if elevation < 0.2 {
    ...
    public func compositionSnowType(elevation: Double, noise: Double) -> Color {
        if elevation < 0.3 {
            // Rocky tundra ground
    ...
    public func compositionRockyType(elevation: Double, noise: Double) -> Color {
        if elevation < 0.3 {
            // Sandy/tan base
    ...
    public func compositionIceType(elevation: Double, noise: Double) -> Color {
        if elevation < 0.4 {
            // Light blue ice
    ...
    #if canImport(UIKit)
    public static func platformColor(from color: Color) -> UIColor {
        UIColor(color)
    }
    #elseif os(macOS)
    public static func platformColor(from color: Color) -> NSColor {
        NSColor(color)
    }
    #endif
}
