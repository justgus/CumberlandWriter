...
struct CitationTests {
    ...
    func createAndFetchImageAttributions() async throws {
        ...
        // Fetch all citations and filter in-memory (avoids #Predicate optional-chain issues)
        let fetch = FetchDescriptor<Citation>(
            sortBy: [SortDescriptor(\.createdAt, order: .forward)]
        )
        let allCitations = try ctx.fetch(fetch)
        let results = allCitations.filter { $0.card?.id == card.id && $0.kind == .image }
    ...
    func editImageAttribution() async throws {
        ...
        let allCitations = try ctx.fetch(FetchDescriptor<Citation>())
        let results = allCitations.filter { $0.card?.id == card.id && $0.kind == .image }
        #expect(results.count == 1)
    ...
    func fetchAllCitationsOrdered() async throws {
        ...
        let allCitations = try ctx.fetch(fetch)
        let results = allCitations.filter { $0.card?.id == card.id }
