...
struct ImageProcessingServiceTests {
    ...
    func testGenerateThumbnailCustomSize() {
        ...
        // Verify thumbnail data is smaller than original (it was scaled down)
        if let thumbnail = thumbnailData {
            #expect(thumbnail.count < imageData.count, "Thumbnail data should be smaller than original")
    ...
    func testGenerateThumbnailDefaultSize() {
        ...
        // Verify thumbnail data is smaller than original
        if let thumbnail = thumbnailData {
            #expect(thumbnail.count < imageData.count, "Default thumbnail data should be smaller than original")

            // Verify it's valid image data
            #if os(macOS)
            #expect(NSImage(data: thumbnail) != nil, "Default thumbnail should be valid NSImage")
            #else
            #expect(UIImage(data: thumbnail) != nil, "Default thumbnail should be valid UIImage")
            #endif
        }
    }
