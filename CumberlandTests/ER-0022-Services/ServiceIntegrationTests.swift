...
@Suite("Service Integration Tests", .serialized)
@MainActor
struct ServiceIntegrationTests {
    ...
    func testServiceContainerInitialization() throws {
        ...
        _ = container.structureRepository
        _ = container.queryService.getAllCards()

        // Verify all services are initialized
        _ = container.cardOperations
        _ = container.relationshipManager
        _ = container.imageProcessing

        // Verify context is accessible
        #expect(container.modelContext === context, "ModelContext should match initialization context")
    }

    @Test("ServiceContainer created from test context has working repository")
    func testServiceContainerFromTestContext() throws {
        let context = try createTestContext()
        let container = ServiceContainer(modelContext: context)
        ...
        let found = allCards.contains { $0.name == "Service Test Card" }
        #expect(found, "Repository should find the inserted card")
    ...
    func testEdgeRepositoryFetchForCard() throws {
        ...
        let edges = edgeRepo.fetchAll(for: hero)
    ...
    func getOrCreateRelationType(context: ModelContext, code: String, forward: String, backward: String) throws -> RelationType {
        ...
        // Create new
        let relationType = RelationType(code: code, forwardLabel: forward, inverseLabel: backward)
